              ...
              <body>
                    ...
                    <tr>
                      <td>
                        <div class="quotation-info">
                          ...
                          <p><strong>Due Date:</strong> ${ format(quotationData.valid_till,'dd-MM-yyyy') }</p>
                        </div>
                ...
                <div class="footer">
                  ...
                  <div class="total" style="font-size:14px">
                    <p>Total sem Imposto: ${quotationData.items.reduce((sum, item) => sum + parseFloat(formatarMoeda.format(item.amount)), 0).toFixed(2)}</p>
                    <p>Valor do Imposto: ${quotationData.items.reduce((sum, item) => sum + parseFloat(formatarMoeda.format(item.amount)), 0).toFixed(2)}</p>
                    <p>Total do Desconto: ${quotationData.items.reduce((sum, item) => sum + parseFloat(formatarMoeda.format(item.amount)), 0).toFixed(2)}</p>
                    <p>Grand Total: ${quotationData.items.reduce((sum, item) => sum + parseFloat(formatarMoeda.format(item.amount)), 0).toFixed(2)}</p>
                  </div>
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        if (quotationData.submit_on_creation === 1) {
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      <Layout style={{ flex: 1, padding: 16 }}>
        {!criarFactura && (
          <View style={{ flex: 1 }}>
            <Text category="h6">{t('home.title_quotations')}</Text>
