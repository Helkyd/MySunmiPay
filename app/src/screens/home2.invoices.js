  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const searchParams0 = { username: userInfo.email };
        await call.get('angola_erp.api.invoices.contarFacturas', searchParams0)
          .then((result) => {
            console.log('**** CONTAGEM DE FACTURAs..... ');
            setNumerodeFacturas(result.message);
          })
          .catch(handleError);
              ...
              <Layout style={{ padding: 8 }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10 }}>
                  ...
                  <Button
                    ...
                    disabled={isLoading}
                  >
                    {isLoading ? <ActivityIndicator color="white" /> : t('home.refresh')}
                  </Button>
                </View>
                {isLoading ? (
                  <Layout style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
